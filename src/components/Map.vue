<template>
    <div>
        <div v-if="showPrompt">
            <PromptPlaceDeletion
                v-on:cancel="setHideDialogue"
                v-on:continue="deletePlace"
                v-bind:question="msg"
            ></PromptPlaceDeletion>
        </div>
        <vl-map
            @click="
                promptAddForm($event.coordinate)
                ...
                <vl-layer-vector>
                    ...
                    >
                        ...
                        >
                            <template>
                                <PlaceCard
                                    v-on:close="selectedFeatures = []"
                                    v-on:delete="
                                        promptPlaceDeletion(feature.properties)
                                    "
...
export default {
    ...
    methods: {
        ...
        promptAddForm(coord) {
            if (this.isdrawing) EventBus.$emit('addplace', coord)
        },
        resetZoomAndCenter() {
            this.center = [0, 0]
            this.zoom = 2
        },
        ...mapActions('place', ['getAllPlaces']),
    },
